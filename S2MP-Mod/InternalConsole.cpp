...
void drawDevelopmentInfo(int windowW, int windowH) {
    ...
    if (font) {
        Functions::_R_AddCmdDrawText("S2MP-MOD PROTOTYPE BUILD", 0x7fffffff, font, 0, 0, font->pixelHeight, 2, 1 + font->pixelHeight, 1.0f, 1.0f, 0.0f, watermarkCol, 0);
...
void R_EndFrame_hookfunc() {
    int winWidth = *(int*)(GameUtil::base + 0x1CFF748); //this isnt actually window size i think its bink player size
    int winHeight = *(int*)(GameUtil::base + 0x1CFF74C);
...
void renderHookInit() {
    void* target = (void*)(GameUtil::base + 0x8B01C0);
    MH_Initialize();
